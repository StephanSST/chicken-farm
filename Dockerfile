...
RUN wget https://download.tinkerforge.com/apt/$(. /etc/os-release; echo $ID)/tinkerforge.gpg -q -O - | tee /etc/apt/trusted.gpg.d/tinkerforge.gpg > /dev/null
RUN echo "deb https://download.tinkerforge.com/apt/$(. /etc/os-release; echo $ID $VERSION_CODENAME) main" | tee /etc/apt/sources.list.d/tinkerforge.list
RUN apt update
